...
class Naive_Jdatalog {
	...
	static class Expr implements Comparable<Expr> {
		...
		boolean unify(Expr that, Map<String, String> bindings) {
			// System.out.print(" Unifying " + this + " against " + that + "; bindings: " +
			// toString(bindings));
	...
	private void matchGoals2(final Collection<Expr> facts, List<Expr> goals, Map<String, String> bindings, Expr head,
			List<Expr> results, List<Map<String, String>> answers) {
		if (goals.isEmpty())
			return;
	...
	public static void startup_naive(String[] args) {
		...
		try {
			if (args.length > 0) {
				...
				try (Reader reader = new BufferedReader(
						new FileReader("C:\\Users\\admin\\eclipse-workspace\\DATALOG\\clique10withprob.txt"))) {
		...
		/*
		 * //This is how you would use the fluent API: JDatalog jDatalog = new
		 * JDatalog();
		 * 
		 * jDatalog.add(new Expr("parent", "a", "aa")); jDatalog.add(new Expr("parent",
		 * "a", "ab")); jDatalog.add(new Expr("parent", "aa", "aaa")); jDatalog.add(new
		 * Expr("parent", "aa", "aab")); jDatalog.add(new Expr("parent", "aaa",
		 * "aaaa")); jDatalog.add(new Expr("parent", "c", "ca"));
		 * 
		 * jDatalog.add(new Rule(new Expr("ancestor", "X", "Y"), new Expr("parent", "X",
		 * "Z"), new Expr("ancestor", "Z", "Y"))); jDatalog.add(new Rule(new
		 * Expr("ancestor", "X", "Y"), new Expr("parent", "X", "Y")));
		 * 
		 * List<Map<String, String>> answers = jDatalog.answerQuery(new Expr("ancestor",
		 * "aa", "X"));
		 */
	}
	 ...
	 * the current one. It actually only needs containsKey(), put() and get() so
	 * I've replaced the implementations of the other methods to throw
	 * UnsupportedOperationException I suppose it isn't strictly necessary that it
	 * implements the Map<> interface, but here we are.
	 */
