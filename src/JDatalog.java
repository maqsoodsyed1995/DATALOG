...
class JDatalog {
	...
	static class Expr {
		...
		// Regex for terms that contain non-alphanumeric charaters and should be quoted
		// in toString()
	 ...
	 * #execute(Reader, QueryOutput) a QueryOutput object that will be used to
	 * display all the results from the separate queries, with their goals.
	 ...
	 */
	public List<Map<String, String>> execute(Reader reader, QueryOutput output)
			throws DatalogException, ParseException {
		...
		try {
			...
			scan.commentChar('%'); // Prolog-style % comments; slashSlashComments and slashStarComments can stay as
									// well.
			scan.quoteChar('"');
			scan.quoteChar('\'');
			// scan.parseNumbers(); // WTF? You can't disable parsing of numbers unless you
			// reset the syntax (http://stackoverflow.com/q/8856750/115589)
			scan.nextToken();
	...
	private List<Map<String, String>> parseStmt(StreamTokenizer scan, List<Expr> goals)
			throws DatalogException, ParseException {
	...
	public List<Map<String, String>> query(List<Expr> goals) throws IOException {
		...
		try {
			...
			Collection<Expr> dataset = buildDatabase(new HashSet<Expr>(edb), idb);
			if (debugEnable) {
	...
	/*
	 * Returns a list of rules that are relevant to the query. If for example you're
	 * quering employment status, you don't care about family relationships, etc.
	 * The advantages of this of this optimization becomes bigger the complexer the
	 * rules get.
	 */
	...
	/*
	 * Basically, this constructs the dependency graph for semi-naive evaluation: In
	 * the returned map, the string is a predicate in the rules' heads that maps to
	 * a collection of all the rules that have that predicate in their body so that
	 * we can easily find the rules that are affected when new facts are deduced in
	 * different iterations of buildDatabase(). For example if you have a rule p(X)
	 * :- q(X) then there will be a mapping from "q" to that rule so that when new
	 * facts q(Z) are deduced, the rule will be run in the next iteration to deduce
	 * p(Z)
	 */
	...
	private List<Expr> matchRule2(final Set<Expr> facts, Rule rule) {
		...
		try {
			...
			// For each match found, substitute the bindings into the head to create a new
			// fact.
	...
	void matchGoals(final Set<Expr> facts, List<Expr> goals, Map<String, String> bindings, Rule rule,
			List<Expr> results, List<Expr> matchedGoals) {
		if (goals.isEmpty())
			return;
	...
	private Collection<StackMap<String, String>> matchGoals2(List<Expr> goals, final Collection<Expr> facts,
			StackMap<String, String> bindings) {
		// First goal
		...
		// If the fact matches: If it is the last/only goal then we can return the
		// bindings
	...
	public boolean delete(List<Expr> goals) throws IOException {
		...
		try {
			...
			List<Expr> facts = answers.stream()
					// Create a new StackMap from the Map in the answer (StackMap doesn't implement
					// Map anymore)
					.map(StackMap::new)
	...
	// If you want to do profiling, run the file several times to get finer grained
	// results.
	...
	public static void startup_seminaive(String... args) {
		...
		try {
			if (args.length > 0) {
				try {
					...
					for (int run = 0; run < NUM_RUNS; run++) {
						...
						for (String arg : args) {
							...
							try (Reader reader = new BufferedReader(new FileReader(
									"C:\\Users\\admin\\eclipse-workspace\\DATALOG\\clique10withprob.txt"))) {
					...
					if (NUM_RUNS > 1) {
						// .out.println("Profile for running " + toString(Arrays.asList(args)) + ";
						// NUM_RUNS=" + NUM_RUNS);
						Profiler.keySet().stream().sorted().forEach(key -> {
							...
							// .out.println(String.format("%-20s time: %10.4fms; total: %12.2fms; count:
							// %d", key, time, total, count));
						});
					}
				...
				while (true) {
					try {
						...
						// If answers is null, the line passed to `jDatalog.query(line)` was a fact or a
						// rule, not a query.
						// If answers is empty, then it was a query that doesn't have any answers, so
						// the output is "No."
						// If the answers are empty maps, then it was the type of query that only wanted
						// a yes/no answer, like
		...
		/*
		 * //This is how you would use the fluent API: try { JDatalog jDatalog = new
		 * JDatalog(); jDatalog.fact("parent", "a", "aa") .fact("parent", "a", "ab")
		 * .fact("parent", "aa", "aaa") .fact("parent", "aa", "aab") .fact("parent",
		 * "aaa", "aaaa") .fact("parent", "c", "ca"); jDatalog.rule(expr("ancestor",
		 * "X", "Y"), expr("parent", "X", "Z"), expr("ancestor", "Z", "Y"))
		 * .rule(expr("ancestor", "X", "Y"), expr("parent", "X", "Y"))
		 * .rule(expr("sibling", "X", "Y"), expr("parent", "Z", "X"), expr("parent",
		 * "Z", "Y")) .rule(expr("related", "X", "Y"), expr("ancestor", "Z", "X"),
		 * expr("ancestor", "Z", "Y")); List<Map<String, String>> answers =
		 * jDatalog.query(expr("ancestor", "aa", "X")); answers.stream().forEach(answer
		 ...
		 * deletes parent(aa,aaa) and parent(aaa,aaaa)
		 * System.out.println("After Deletion: " + toString(jDatalog.edb)); answers =
		 * jDatalog.query(expr("ancestor", "aa", "X")); answers.stream().forEach(answer
		 * -> System.out.println(" -> " + toString(answer))); } catch (DatalogException
		 * e) { e.printStackTrace(); }
		 */

		/*
		 * // The JDatalog.execute(String) method runs queries directly. try{
		 * QueryOutput qo = new StandardQueryOutput(); JDatalog jDatalog = new
		 * JDatalog(); jDatalog.execute("foo(bar). foo(baz).", qo);
		 * jDatalog.execute("foo(What)?", qo); } catch (DatalogException e) {
		 * e.printStackTrace(); }
		 */
	...
	/*
	 * Map that has a parent Map where it looks up a value if the value is not in
	 * the current one. It actually only needs containsKey(), put() and get(). It
	 * originally implemented the Map<> interface, but there were practical
	 * considerations, so it doesn't anymore.
	 */
	...
	/*
	 * Profiling class. I had to write my own because I didn't want to pull in any
	 * external dependencies. `buckets` maps the name of a bucket to a List of
	 * elapsed times so that you can have multiple timers under different names.
	 */
